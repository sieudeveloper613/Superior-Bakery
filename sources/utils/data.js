import { images } from '../themes/images'
import { icons } from '../themes/icons'


export const reviews = [
    {
        id: 1, 
        label: 'Giới thiệu',
        body: 'Nguồn gốc của bánh mì S',
        content: 'Lorem ipsum dolor sit amet consectetur. Porta pellentesque nulla libero commodo elementum purus mi egestas mauris. Faucibus vel facilisis purus malesuada tellus morbi nulla. At tempus ac praesent quam vel ridiculus viverra tristique. Commodo et volutpat quam mauris eu sed tempus sit. Porta pulvinar lacinia elit proin.',
        image: images.SUPERIOR_LOGO,
    },
    {
        id: 2, 
        label: 'Khởi đầu',
        body: 'Khi mà Superior ra đời',
        content: 'Lorem ipsum dolor sit amet consectetur. Porta pellentesque nulla libero commodo elementum purus mi egestas mauris. Faucibus vel facilisis purus malesuada tellus morbi nulla. At tempus ac praesent quam vel ridiculus viverra tristique. Commodo et volutpat quam mauris eu sed tempus sit. Porta pulvinar lacinia elit proin.',
        image: images.SUPERIOR_LOGO,
    },
    {
        id: 3, 
        label: 'Vươn lên',
        body: 'Thất bại rồi lại thành công',
        content: 'Lorem ipsum dolor sit amet consectetur. Porta pellentesque nulla libero commodo elementum purus mi egestas mauris. Faucibus vel facilisis purus malesuada tellus morbi nulla. At tempus ac praesent quam vel ridiculus viverra tristique. Commodo et volutpat quam mauris eu sed tempus sit. Porta pulvinar lacinia elit proin.',
        image: images.SUPERIOR_LOGO
    },
    {
        id: 4, 
        label: 'Tương lai',
        body: 'Thế hệ mới của S',
        content: 'Lorem ipsum dolor sit amet consectetur. Porta pellentesque nulla libero commodo elementum purus mi egestas mauris. Faucibus vel facilisis purus malesuada tellus morbi nulla. At tempus ac praesent quam vel ridiculus viverra tristique. Commodo et volutpat quam mauris eu sed tempus sit. Porta pulvinar lacinia elit proin.',
        image: images.SUPERIOR_LOGO

    },
]

export const sellest = [{
    "id": 1,
    "foodName": "Praesent id massa id nisl venenatis lacinia. Aenean sit amet justo. Morbi ut odio.",
    "sold": 12,
    "price": "$6.10",
    "image": "data:image/png;base64,",
    "createDate": "29/06/2022"
  }, {
    "id": 2,
    "foodName": "Nullam sit amet turpis elementum ligula vehicula consequat. Morbi a ipsum. Integer a nibh.",
    "sold": 47,
    "price": "$3.83",
    "image": "data:image/png;base64,",
    "createDate": "23/11/2022"
  }, {
    "id": 3,
    "foodName": "Curabitur at ipsum ac tellus semper interdum. Mauris ullamcorper purus sit amet nulla. Quisque arcu libero, rutrum ac, lobortis vel, dapibus at, diam.",
    "sold": 6,
    "price": "$6.55",
    "image": "data:image/png;base64,",
    "createDate": "25/06/2022"
  }, {
    "id": 4,
    "foodName": "Aenean fermentum. Donec ut mauris eget massa tempor convallis. Nulla neque libero, convallis eget, eleifend luctus, ultricies eu, nibh.",
    "sold": 83,
    "price": "$9.24",
    "image": "data:image/png;base64,",
    "createDate": "21/06/2022"
  }, {
    "id": 5,
    "foodName": "In hac habitasse platea dictumst. Morbi vestibulum, velit id pretium iaculis, diam erat fermentum justo, nec condimentum neque sapien placerat ante. Nulla justo.",
    "sold": 27,
    "price": "$6.75",
    "image": "data:image/png;base64,",
    "createDate": "29/11/2022"
  }, {
    "id": 6,
    "foodName": "Phasellus in felis. Donec semper sapien a libero. Nam dui.",
    "sold": 75,
    "price": "$7.13",
    "image": "data:image/png;base64,",
    "createDate": "29/12/2022"
  }, {
    "id": 7,
    "foodName": "Sed ante. Vivamus tortor. Duis mattis egestas metus.",
    "sold": 81,
    "price": "$3.31",
    "image": "data:image/png;base64,",
    "createDate": "07/01/2023"
  }, {
    "id": 8,
    "foodName": "Morbi non lectus. Aliquam sit amet diam in magna bibendum imperdiet. Nullam orci pede, venenatis non, sodales sed, tincidunt eu, felis.",
    "sold": 15,
    "price": "$1.91",
    "image": "data:image/png;base64,",
    "createDate": "16/01/2023"
  }, {
    "id": 9,
    "foodName": "Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Vivamus vestibulum sagittis sapien. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus.",
    "sold": 97,
    "price": "$4.66",
    "image": "data:image/png;base64,",
    "createDate": "16/03/2023"
  }, {
    "id": 10,
    "foodName": "Quisque id justo sit amet sapien dignissim vestibulum. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Nulla dapibus dolor vel est. Donec odio justo, sollicitudin ut, suscipit a, feugiat et, eros.",
    "sold": 12,
    "price": "$9.38",
    "image": "data:image/png;base64,",
    "createDate": "08/01/2023"
}]

export const categories = [
  {
    id: 1,
    label: 'Bánh kem',
    image: icons.CAKE_ICON,
  },
  {
    id: 2,
    label: 'Bánh mì',
    image: icons.BURGER_ICON,
  },
  {
    id: 3,
    label: 'Trà',
    image: icons.DRINK_ICON,
  },
  {
    id: 4,
    label: 'Khác',
    image: icons.DISH_ICON,
  },
]

export const menu = [
  {
    id: 1,
    icon: 'home',
    name: 'Trang chủ'
  },
  {
    id: 2,
    icon: 'user',
    name: 'Thông tin người dùng'
  },
  {
    id: 3,
    icon: 'bell',
    name: 'Thông báo'
  },
  {
    id: 4,
    icon: 'heart',
    name: 'Yêu thích'
  },
  {
    id: 5,
    icon: 'archive',
    name: 'Giỏ hàng'
  },
  {
    id: 6,
    icon: 'tag',
    name: 'Mã quà tặng'
  },
  {
    id: 7,
    icon: 'award',
    name: 'Về chúng tôi'
  },
  {
    id: 8,
    icon: 'log-out',
    name: 'Đăng xuất'
  }
]

export const filterProduct = {
  filterByCategory: [
    {
      id: 1,
      category: 'cake',
      name: 'Bánh kem',
    },
    {
      id: 2,
      category: 'bread',
      name: 'Bánh mì',
    },
    {
      id: 3,
      category: 'tea',
      name: 'Trà',
    },
  ],
  filterByPrice: [
    {
      id: 1,
      category: 'lowest',
      name: 'Giá thấp nhất',
    },
    {
      id: 2,
      category: 'highest',
      name: 'Giá cao nhất',
    },
  ],
  filterByType: [
    {
      id: 1,
      category: 'sellest',
      name: 'Bán chạy nhất',
    },
    {
      id: 2,
      category: 'sale',
      name: 'Đang giảm giá',
    },
    {
      id: 3,
      category: 'newest',
      name: 'Mới',
    },
  ],
  filterByTime: [
    {
      id: 1,
      type: 'morning',
      name: 'Sáng'
    },
    {
      id: 2,
      type: 'noon',
      name: 'Trưa'
    },
    {
      id: 3,
      type: 'afternoon',
      name: 'Chiều'
    },
    {
      id: 4,
      type: 'evening',
      name: 'Tối'
    },

  ]
}

export const notificationType = [
  {
    id: 0,
    type: 'all',
    name: 'Tất cả',
  },
  {
    id: 1,
    type: 'yours',
    name: 'Của bạn',
  },
  {
    id: 2,
    type: 'event',
    name: 'Sự kiện',
  },
]

export const profileMenu = [
  {
    id: 0,
    icon: 'settings',
    name: 'Cật nhập thông tin'
  },
  {
    id: 1,
    icon: 'navigation',
    name: 'Địa chỉ giao hàng'
  },
  {
    id: 2,
    icon: 'lock',
    name: 'Đổi mật khẩu'
  },
  {
    id: 3,
    icon: 'lock',
    name: 'Lịch sử mua hàng'
  },
  {
    id: 4,
    icon: 'map-pin',
    name: 'Vị trí cửa hàng'
  },
  {
    id: 5,
    icon: 'download-cloud',
    name: 'Cật nhật ứng dụng'
  },
  {
    id: 6,
    icon: 'log-out',
    name: 'Đăng xuất tài khoản'
  },  
]

export const addressType = [
  {
    id: 0, 
    type: 'default', 
    name: 'Địa chỉ mặc định'
  },
  {
    id: 1, 
    type: 'home', 
    name: 'Nhà riêng'
  },
  {
    id: 2, 
    type: 'company', 
    name: 'Công ty'
  },
  {
    id: 3, 
    type: 'other', 
    name: 'Khác'
  },
]

export const stores = [
  {
    id: 1,
    store: 'Chi nhánh 1',
    name: 'Tiệm bánh mì Superior',
    ward: 'Quận 1',
    label: 'Chi nhánh 1 (quận 1)',
    address: '123 Nguyễn Văn A, Phường B, Quận C, Thành phố D',
    location: {
      latitude: '00000',
      longtitude: '0000'
    },
    image: images.SUPERIOR_LOGO
  },
  {
    id: 2,
    store: 'Chi nhánh 2',
    name: 'Tiệm bánh mì Superior',
    ward: 'Quận 2',
    label: 'Chi nhánh 2 (quận 2)',
    address: '123 Nguyễn Văn A, Phường B, Quận C, Thành phố D',
    location: {
      latitude: '00000',
      longtitude: '0000'
    },
    image: images.SUPERIOR_LOGO
  },
  {
    id: 3,
    store: 'Chi nhánh 3',
    name: 'Tiệm bánh mì Superior',
    ward: 'Quận Bình Thạnh',
    label: 'Chi nhánh 3 (quận Bình Thạnh)',
    address: '123 Nguyễn Văn A, Phường B, Quận C, Thành phố D',
    location: {
      latitude: '00000',
      longtitude: '0000'
    },
    image: images.SUPERIOR_LOGO
  },
  {
    id: 4,
    store: 'Chi nhánh 4',
    name: 'Tiệm bánh mì Superior',
    ward: 'Quận Gò Vấp',
    label: 'Chi nhánh 4 (quận Gò Vấp)',
    address: '123 Nguyễn Văn A, Phường B, Quận C, Thành phố D',
    location: {
      latitude: '00000',
      longtitude: '0000'
    },
    image: images.SUPERIOR_LOGO
  },
  {
    id: 5,
    store: 'Chi nhánh 5',
    name: 'Tiệm bánh mì Superior',
    ward: 'Quận 5',
    label: 'Chi nhánh 5 (quận 5)',
    address: '123 Nguyễn Văn A, Phường B, Quận C, Thành phố D',
    location: {
      latitude: '00000',
      longtitude: '0000'
    },
    image: images.SUPERIOR_LOGO
  },
  {
    id: 6,
    store: 'Chi nhánh 6',
    name: 'Tiệm bánh mì Superior',
    ward: 'Quận 12',
    label: 'Chi nhánh 6 (quận 12)',
    address: '123 Nguyễn Văn A, Phường B, Quận C, Thành phố D',
    location: {
      latitude: '00000',
      longtitude: '0000'
    },
    image: images.SUPERIOR_LOGO
  },
]

export const notification = [
  {
    id: 1,
    type: 1, // 1 is yours, 2 is event
    message: 'Lorem ipsum dolor sit amet sit amet elit que consectetur. Molestie lacus eu.',
    detail: '',
    image: images.AVATAR,
    isView: 1,
    sendAt: '10 phút'
  },
  {
    id: 2,
    type: 2,
    message: 'Lorem ipsum dolor sit amet sit amet elit que consectetur. Molestie lacus eu.',
    detail: '',
    image: images.AVATAR,
    isView: 0,
    sendAt: '50 phút'
  },
  {
    id: 3,
    type: 2,
    message: 'Lorem ipsum dolor sit amet sit amet elit que consectetur. Molestie lacus eu.',
    detail: '',
    image: images.AVATAR,
    isView: 1,
    sendAt: '3 ngày trước'
  },
  {
    id: 4,
    type: 1,
    message: 'Lorem ipsum dolor sit amet sit amet elit que consectetur. Molestie lacus eu.',
    detail: '',
    image: images.AVATAR,
    isView: 1,
    sendAt: '1 tuần trước'
  },
  {
    id: 5,
    type: 1,
    message: 'Lorem ipsum dolor sit amet sit amet elit que consectetur. Molestie lacus eu.',
    detail: '',
    image: images.AVATAR,
    isView: 1,
    sendAt: '2 tuần trước'
  },
  {
    id: 6,
    type: 2,
    message: 'Lorem ipsum dolor sit amet sit amet elit que consectetur. Molestie lacus eu.',
    detail: '',
    image: images.AVATAR,
    isView: 0,
    sendAt: '1 tháng trước'
  },
]

export const orderedTime = [
  {
    id: 0,
    label: 'Tất cả hóa đơn',
    value: 'all'
  },
  {
    id: 1,
    label: 'Hóa đơn trong 3 ngày trước',
    value:'3',
  },
  {
    id: 0,
    label: 'Hóa đơn trong 7 ngày trước',
    value: '7'
  },
  {
    id: 0,
    label: 'Hóa đơn trong 1 tháng trước',
    value: '30'
  },
  {
    id: 0,
    label: 'Hóa đơn trong 3 tháng trước',
    value: '90'
  },
]